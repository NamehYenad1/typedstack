...
import "../app/globals.css"; // Import global styles to access CSS variables
...
const lightModeColors = {
  ...
  "--card": "hsl(var(--slate-50))",
  "--card-foreground": "hsl(var(--slate-900))",
  "--popover": "hsl(var(--slate-50))",
  "--popover-foreground": "hsl(var(--slate-900))",
  "--primary": "hsl(var(--slate-900))",
  "--primary-hover": "hsl(var(--slate-800))",
  ...
  "--muted-foreground": "hsl(var(--slate-500))",
  "--accent": "hsl(var(--slate-100))",
...
const darkModeOverrides = {
  ...
  "--primary": "hsl(var(--slate-50))",
  "--primary-hover": "hsl(var(--slate-200))",
  "--primary-foreground": "hsl(var(--slate-900))",
  ...
  "--destructive": "hsl(var(--red-900))", // Note: Needs --red-900 defined in :root
  "--destructive-foreground": "hsl(var(--slate-50))",
...
const ColorSwatch: React.FC<ColorSwatchProps> = ({
  ...
  // Determine the actual background color to apply
  // For HSL values directly, we use them. For aliases, we rely on CSS variables.
  const isAlias = value.startsWith("hsl(var(");
  const backgroundColor = isAlias ? value : `hsl(${value})`;
  ...
  return (
    <div
      style={{
        ...
        alignItems: "center",
        minWidth: "150px",
        backgroundColor: theme === "dark" ? "#222" : "#fff", // Swatch background
        color: theme === "dark" ? "#eee" : "#333", // Text color
      }}
    >
      <div
        style={{
          width: "100px",
          height: "50px",
          backgroundColor: backgroundColor, // Apply the color variable
          border: "1px solid #eee",
          borderRadius: "4px",
          marginBottom: "8px",
        }}
      ></div>
      <code style={{ fontSize: "0.8em", wordBreak: "break-all" }}>{name}</code>
      <span style={{ fontSize: "0.7em", color: "#666" }}>{value}</span>
    </div>
  );
};
...
const ColorPalette: React.FC<{
  ...
  <div style={{ display: "flex", flexWrap: "wrap" }}>
    {Object.entries(colors).map(([name, value]) => (
      <ColorSwatch key={name} name={name} value={value} theme={theme} />
    ))}
  </div>
);
...
const meta: Meta = {
  ...
  decorators: [
    (Story, context) => {
      // Access the theme from storybook-addon-data-theme-switcher
      const theme = context.globals.dataTheme || "light";
      // Apply the theme to the preview body
      React.useEffect(() => {
        document.documentElement.setAttribute("data-theme", theme);
        // Optional: Apply to body as well if needed by specific styles
...
export const LightTheme: Story = {
  ...
  render: (args) => (
    <div>
      <h2>Light Theme Colors (:root)</h2>
      <ColorPalette {...args} />
    </div>
  ),
};

export const DarkThemeOverrides: Story = {
  args: {
    colors: darkModeOverrides,
    theme: "dark",
  },
  render: (args) => (
    // Ensure the container uses dark theme variables
    <div
      data-theme="dark"
      style={{
        padding: "20px",
    ...
    >
      <h2>Dark Theme Overrides ([data-theme=&quot;dark&quot;])</h2>
      <p style={{ fontSize: "0.9em", marginBottom: "10px" }}>
        These variables override the base :root variables when{" "}
        <code>data-theme=&quot;dark&quot;</code> is applied.
      </p>
      <ColorPalette {...args} />
    </div>
  ),
...
export const CombinedView: Story = {
  render: () => (
    <div>
      <h1>Color Palette</h1>
      <p>
        This story displays the color variables defined in{" "}
      ...
      <section>
        ...
        >
          <ColorPalette colors={lightModeColors} theme="light" />
        </div>
      </section>
      ...
      <section>
        ...
        >
          ...
          <ColorPalette colors={darkModeOverrides} theme="dark" />
        </div>
      </section>
    </div>
  ),
